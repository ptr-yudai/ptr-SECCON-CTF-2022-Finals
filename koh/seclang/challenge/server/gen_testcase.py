...
with sqlite3.connect('teams.db') as conn:
    ...
    # history: テストケース結果のログ
    # score: 現在のスコア
    # pwncnt: 何回フラグを盗まれたか（テストケースを無効にする回数）
    # pwned: どのidのチームのフラグを盗んだか
    # timestamp: 最後にコンテナの更新に成功した時間
    # flag: 現在のフラグ
    cur.execute("""
CREATE TABLE teams(
  ...
  pwncnt INTEGER DEFAULT 0,
  pwned STRING DEFAULT '[]',
  timestamp STRING,
  flag STRING DEFAULT 'SECCON{!empty flag: report to admin!}'
)
    """)
    # テストケーステーブル
    cur.execute("""
CREATE TABLE testcases(
  ...
  input STRING
    ...
    for team in teams:
        team.append('2000/01/01 00:00:00')
    cur.executemany("INSERT INTO teams(name, token, timestamp) VALUES(?, ?, ?)",
                    teams)
