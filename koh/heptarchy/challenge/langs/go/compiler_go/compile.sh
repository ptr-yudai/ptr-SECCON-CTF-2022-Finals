...
# Build image
if [[ "$(docker images -q compiler_go 2> /dev/null)" == "" ]]; then
    echo -e "\e[33;1m[+] First time to compile. Building container...\e[m"
    docker build . -t compiler_go
fi
...
# Compile
cp "$CODE" "$WORKDIR/main.go"
timeout -sKILL 10 \
        docker run --rm \
        -v "$WORKDIR:/tmp" \
        compiler_go /tmp/main.go
...
echo -e "objcopy -O binary --only-section=.text mission-4 mission-4.text"
echo -e "./compare mission-4.text output.text\e[m"
